...
export const getUsers = async (
  ...
  limit: number = 10,
  search: string = ""
): Promise<Omit<User, "created_at" | "updated_at" | "password">[]> => {
...
export const getUserById = async (
  ...
  Omit<User, "created_at" | "updated_at" | "password"> | undefined
> => {
  const doc = await usersCollection.doc(id).get();
  if (!doc.exists) {
    return undefined;
  }
  const { created_at, updated_at, password, ...userResponse } =
    doc.data() as User;
  return userResponse;
};
...
> => {
  ...
  await docRef.update({ name, updated_at: new Date() });
